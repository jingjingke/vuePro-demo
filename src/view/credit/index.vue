	    ...
	    data () {
	        return {
	        	...
	        	datas:[
	        		...
	        		{cls:'icon-scan',		tit:'人脸识别',		push:'/credit/scan',		isOk:false, 	param:'userScan'},
	        		{cls:'icon-phone',		tit:'手机认证',		push:'/credit/mobile',		isOk:false, 	param:'userPhone'},
	        		...
	        		{cls:'icon-shebao',		tit:'社保授权',		push:'/credit/sbsqCity',	isOk:false, 	param:'userFund'},
	        		{cls:'icon-chuxu',		tit:'储蓄卡流水导入',push:'/credit/waterBank',	isOk:false, 	param:'userChuxu'},
	        		{cls:'icon-zhima',		tit:'芝麻信用授权',	push:'/credit/zhima',		isOk:false, 	param:'userZhima'},
					{cls:'icon-bank',		tit:'银行卡信息',	push:'/credit/bankInfo',	isOk:false, 	param:'userBank'},
					{cls:'icon-credit',		tit:'查询央行征信',	push:'/creditReport',		isOk:false, 	param:'userCredit'},
					{cls:'icon-alipay',		tit:'支付宝认证',	push:'/credit/alipay',		isOk:false, 	param:'userIdpic'},
	    ...
	    mounted:function(){
	    	...
	    	for(var i =0; i < this.datas.length; i++){
				if(localStorage[this.datas[i].param] === undefined || localStorage[this.datas[i].param] === null){
	    			this.datas[i].isOk = false;
	    		}else	this.datas[i].isOk = true;
			}
	    }
	}	
</script>