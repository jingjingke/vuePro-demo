<template>
	<div class="container">
		...
		<template v-if='!hasCard'>
			<p class="listTop bg-blue">目前支持16家银行的储蓄卡，<span class="blue txtline" @click="$router.push('/credit/support')">查看支持银行</span></p>
	...
	export default {
	    ...
	    methods :{
	        nextStep(){
	        	...
	        	if(checkCard.test(this.data.cardId) == false)			this.callDialog("银行卡号格式不对");
	        	else if(checkTel.test(this.data.phone)==false)			this.callDialog("手机号码格式不对");
	        	else if(this.regDef.test(this.data.province))			this.callDialog("请选择开户省");
