	...
	export default {
	    ...
	    methods :{
	        saveInfo () {
	        	...
	        	else if( checkName.test(this.data.name) == false )			 				this.callDialog("姓名格式错误");
	        	else if( checkId.test(this.data.id) == false ) 								this.callDialog("身份证格式错误");
	        	else if(this.data.selectSex === '') 										this.callDialog("请选择您的性别");
	        	else if(this.data.selectEdu === '') 										this.callDialog("请选择您的学历");
	        	else if(this.data.selectMar === '') 										this.callDialog("请选择婚姻状况");
	        	else if(this.data.selectProy === '') 										this.callDialog("请选择目前所在省");
	        	else if(this.data.selectCity === '' || this.data.selectCity === undefined) this.callDialog("请选择目前所在市");
	        	else if(this.data.selectDist === '' || this.data.selectDist === undefined) this.callDialog("请选择目前所在区");
	        ...
	        setDistList(){
	        	...
	        	this.optionDist =  this.replyData(this.data.selectCity).length == 0 ? [{"code":"无","address":"无"}] : this.replyData(this.data.selectCity);
	    ...
	    filters:{
	    	optionArrobj:function(value,obj){
				...
				if(value=='请选择' || value=== '') return value;
					...
					for(var i = 0; i < obj.length; i ++) newObj[obj[i].code] = obj[i].address;
					return newObj[value];
				}
	    	}
	    },
